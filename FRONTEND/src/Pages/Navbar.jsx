            ...
            {navLinks.map((link) => (
              <div key={link.name} className="relative">
                {link.subLinks ? (
                  ...
                  >
                    {link.name}
                    <span className="absolute left-2 -bottom-1 h-0.5 w-0 bg-linear-to-r from-fuchsia-500 via-blue-500 to-cyan-400 transition-all duration-300 group-hover:w-[calc(100%-16px)] rounded-full"></span>
                  </Link>
                )}
              </div>
            ))}
