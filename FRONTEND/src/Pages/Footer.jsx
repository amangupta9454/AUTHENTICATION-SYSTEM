...
const Footer = () => {
  ...
  return (
    <footer className="relative bg-linear-to-b from-gray-950 to-gray-900 text-gray-100 pt-24 pb-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      {/* CSS Background Animation */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-linear-to-r from-transparent via-indigo-500/10 to-transparent animate-[slide_20s_linear_infinite] bg-size-[200%_100%]" />
        <div className="absolute w-150 h-150 -top-37.5 -left-37.5 bg-linear-to-br from-indigo-500/25 to-teal-400/25 blur-[100px] rounded-full animate-[pulse_10s_ease-in-out_infinite]" />
        <div className="absolute w-125 h-125 -bottom-30 -right-30 bg-linear-to-tr from-rose-500/20 to-amber-400/20 blur-[90px] rounded-full animate-[pulse_12s_ease-in-out_infinite_reverse]" />
      </div>

      {/* Scroll to Top Arrow Button (Fixed) */}
      <button
        onClick={scrollToTop}
        className="fixed bottom-8 right-8 z-50 p-3 bg-linear-to-r from-teal-400 to-indigo-500 text-white rounded-full shadow-lg hover:shadow-teal-400/60 transition-all duration-300 hover:scale-110 group cursor-pointer"
      >
      ...
      {/* Divider */}
      <div className="my-12 w-full h-px bg-linear-to-r from-transparent via-teal-400/80 to-transparent" />
      ...
      <div className="mt-3 text-center text-sm sm:text-base">
        <button onClick={() => setIsModalOpen(true)}
          ...
          Crafted by <span className="font-semibold bg-linear-to-r from-teal-400 via-indigo-500 to-rose-500 text-transparent bg-clip-text animate-[glimmer_10s_linear_infinite]">CodeVeda</span>
        </button>
      </div>
